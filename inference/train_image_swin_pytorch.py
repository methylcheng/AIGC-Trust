"""
基于 PyTorch + Swin Transformer V2 的 AIGC 图像检测模型训练
优势：
1. 层次化特征提取，更适合图像检测
2. 滑动窗口注意力机制，捕捉局部细节
3. 对AI生成的纹理异常更敏感
...
架构：Swin-V2-Tiny (使用ImageNet预训练权重)
- Patch大小: 4×4
- 窗口大小: 7×7
- 隐藏层维度: 96 (Tiny) / 192 (Small) / 384 (Base)
- 输入尺寸: 224×224
"""
import os
import numpy as np
...
def create_dataloader(base_dir, split='train', batch_size=32, max_samples_per_model=None, shuffle=True, use_balanced_sampler=False):
    ...
    dataloader = DataLoader(
        ...
        shuffle=shuffle,
        sampler=sampler,
        num_workers=0,  # Windows下禁用多进程避免共享内存错误
        pin_memory=False,  # Windows下禁用pinned memory减少内存压力
...
def train_swin(epochs=30, batch_size=64, max_train_samples_per_model=None, max_test_samples_per_model=None, use_balanced_sampler=True):
    """
    训练 Swin Transformer V2 模型（高精度全量训练模式）
    ...
    """
    save_path = os.path.join(os.path.dirname(__file__), "models", "image_detector_swin.pth")
    
    print("=" * 70)
    print("开始训练 Swin Transformer V2 AIGC检测模型 (PyTorch)")
    print("=" * 70)
    print("\n模型配置:")
    print("  - 架构: Swin-V2-Tiny")
    print("  - Patch大小: 4×4")
    print("  - 窗口大小: 7×7")
    print("  - 隐藏层: 96维 (Tiny)")
    print("  - 输入尺寸: 224×224")
    ...
    # 创建Swin Transformer网络
    print("\n【步骤 2/4】构建 Swin Transformer V2 模型...")
    print("  使用 torchvision 预训练 Swin-V2-Tiny (ImageNet权重)")
    
    # 使用 torchvision 的预训练 Swin Transformer V2
    swin_model = models.swin_v2_t(weights=models.Swin_V2_T_Weights.IMAGENET1K_V1)
    
    # 修改分类头为2分类
    num_features = swin_model.head.in_features
    swin_model.head = nn.Linear(num_features, 2)
    ...
    print("-" * 70)
    print("提示: Swin Transformer训练初期Loss下降较慢，请耐心等待...")
    print("✨ 启用超高精度训练模式:")
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (images, labels) in enumerate(train_loader):
            ...
            labels = labels.to(device)  # DataLoader已返回tensor，无需转换
            ...
            # 统计（在scheduler.step()之前）
            running_loss += loss.item()
            _, predicted = torch.max(outputs, 1)
            total += labels.size(0)
            correct += (predicted == labels).sum().item()
            
            # 更新学习率（optimizer.step()之后调用）
            scheduler.step()
...
if __name__ == "__main__":
    # ========== 三分之一数据集训练配置 ==========
    print("=" * 70)
    print("🚀 Swin Transformer模型快速训练（1/3数据集）")
    print("=" * 70)
    print("\n💡 训练策略:")
